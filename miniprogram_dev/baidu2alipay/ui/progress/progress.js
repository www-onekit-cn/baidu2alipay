...
Component({
  ...
  didMount: function didMount() {
    ...
    var duration = void 0;
    var curPercent = this.props.percent;
    var timestamp = this.props.duration * this.props.percent;
    if (this.props.color) {
    ...
    my.createSelectorQuery().selectAll('.onekit-progress').boundingClientRect().exec(function (res) {
      my.createSelectorQuery().select('.onekit-progress').boundingClientRect().exec(function (ret) {
        if (_this.props.active) {
          ...
          setTimeout(function () {
            _this._trigger_activeend(detail);
          }, timestamp);
        }
      });
    });
  ...
  methods: {
    _trigger_activeend: function _trigger_activeend(detail, dataset) {
